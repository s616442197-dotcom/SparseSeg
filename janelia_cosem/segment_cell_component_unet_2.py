...
# ============================
thickness=2
dataset = ValidPatchSliceDataset(volume,test_volume_label,negative_volume_label,softnega, patch_size=patchsize, threshold=0.0, num_samples=1000,thickness=thickness,valid_z=valid_z)
...
# ====== 通道扩展卷积 ====== #
class ChannelExpandConv(nn.Module):
    def __init__(self, in_channels, expand_factor=2, downsample=False):
        super().__init__()
        out_channels = in_channels * expand_factor
...
class ChannelTransformer(nn.Module):
    def __init__(self, channels, num_heads=2, num_layers=2, dim_ff=256):
        super().__init__()
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=channels, nhead=num_heads, dim_feedforward=dim_ff, batch_first=True
        )
        self.encoder = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)

    def forward(self, x):
        B, C, H, W = x.shape
        # 每个通道看作一个token（序列长度=C，维度=H×W）
        x = x.view(B, C, H * W)  # [B, C, HW]
        x = x.permute(0, 2, 1)  # [B, HW, C] → 为 batch_first=True 的 transformer 准备输入
        x = self.encoder(x)  # transformer 沿通道方向建模
        x = x.permute(0, 2, 1)
        x = x.view(B, C, H, W)
        return x
# ====== 主网络 ====== #
class ExpandTransformerUNet(nn.Module):
    def __init__(self, in_channels, out_channels=1, base_channels=None):
        super().__init__()
        base_channels = in_channels if base_channels is None else base_channels

        # Stage 1 (下采样 + 扩展)
        self.expand1 = ChannelExpandConv(base_channels, expand_factor=2, downsample=True)
        self.trans1  = ChannelTransformer(base_channels * 2,num_heads=2)

        # Stage 2 (再次下采样 + 扩展)
        self.expand2 = ChannelExpandConv(base_channels * 2, expand_factor=2, downsample=True)
        self.trans2  = ChannelTransformer(base_channels * 4,num_heads=4)
    ...
    def forward(self, x):
        # Encoder
        e1 = self.expand1(x)      # [B, 2C, H/2, W/2]
        e1 = self.trans1(e1)

        e2 = self.expand2(e1)     # [B, 4C, H/4, W/4]
        e2 = self.trans2(e2)

        # Decoder
        d1 = self.up1(e2)         # [B, 2C, H/2, W/2]
        d1 = self.dec1(torch.cat([d1, e1], dim=1))
...
def masked_soft_bce_loss(pred, target, negative_target=None,softnega=None,
    ...
    bce_loss = F.binary_cross_entropy_with_logits(pred, target, weight=weight)
    ...
    loss = bce_loss + 0.0*weighted_loss
...
def smoothness_loss0(pred_mask, input_image, slice_idx=None):
    ...
    return loss
from Loss_func import region_consistency_loss, region_contrast_loss
...
def total_loss_fn(pred, target, input_img, negative_label, softnega, model,
                  ...
                  l1_weight=0.01, high_weight=1.0, low_weight=0.5, thickness=5):
    ...
    correlation = region_consistency_loss(pred, input_img, slice_idx=2 * thickness + 1) + 0.1* region_contrast_loss(pred, input_img, slice_idx=2 * thickness + 1)
...
edge_vol = infer_volume_edges_whole(volume, model, thickness=thickness)
...
def local_variance_3d(volume, win_size=5):
    """
    计算3D体素数据的局部方差
    """
    mean = uniform_filter(volume, size=win_size, mode='reflect')
    mean_sq = uniform_filter(volume ** 2, size=win_size, mode='reflect')
    var = (mean_sq - mean ** 2)/(mean_sq+1e-6)
    return var
def enhance_edge_by_variance(edge_vol, win_size=5):
    """
    用局部方差增强edge_vol：
    1. 计算local variance
    2. 按方差从大到小归一化到[0,1]
    ...
    """
    # 计算局部方差
    var_map = local_variance_3d(edge_vol, win_size=win_size)

    # 归一化（方差大的地方接近1）
    vmin, vmax = var_map.min(), var_map.max()
    var_norm = (var_map - vmin) / (vmax - vmin + 1e-8)

    # 可选：反向归一化（如果想抑制高方差区域）
    var_norm = 1 - var_norm

    # 结合 edge 强度
    edge_new = edge_vol * var_norm

    return edge_new, var_norm
from scipy.io import savemat
# edge_new, var_norm = enhance_edge_by_variance(edge_vol, win_size=10)
edge_new = edge_vol
thresh_value = np.percentile(edge_new, 100* msk_threshold)
vol01 = (edge_new >= max(thresh_value,0.5)).astype(np.uint8)

# vol01=(edge_vol > 0.5).astype(np.uint8)
...
save_volume_with_masks_as_rgb_tiff(volume, edge_vol, test_volume_label_new, f'{mask_name}/remain_mask_{interation_idx}.tif')
