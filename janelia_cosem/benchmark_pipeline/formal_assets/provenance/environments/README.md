...
- `vem_module_pip_freeze.txt`: SparseSeg, SparseSeg-ViT, Vanilla U-Net, the released COSEM-architecture 2D/3D U-Net controls, and related PyTorch code used for the new controls (`torch==2.5.1`, CUDA runtime reported by PyTorch as 12.1 in the timing records).
...
- `cellmap_csc_pip_freeze.txt`: historical exploratory environment snapshot retained as read-only provenance; it was not the execution environment for the reported COSEM-architecture controls.
