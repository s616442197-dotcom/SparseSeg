#!/usr/bin/env python3
"""Parallel cached continuous complete-object policy with unseen-trial LOTO ensembling.
...
# Replace the now-unused sequential constructor with the exact-feature parallel
# implementation. The selector, thresholds, record order and union remain in
# the schema-735 program; only candidate execution is delegated.
import_old = "import argparse\nfrom concurrent.futures import ThreadPoolExecutor\n"
import_new = (
    "import argparse\n"
    "from concurrent.futures import ThreadPoolExecutor\n"
    "import fast_parallel_oof_runtime_v800 as fast_core\n"
)
if source.count(import_old) != 1:
    raise RuntimeError("v800 generated import anchor changed")
source = source.replace(import_old, import_new)

call_old = """    records, seed_summary = construct_runtime_records(
        probability, raw, base, negative, args.feature_volume
    )
"""
call_new = """    records, seed_summary = fast_core.construct_runtime_records(
        probability,
        raw,
        base,
        negative,
...
if source.count(call_old) != 1:
    raise RuntimeError("v800 runtime-constructor call anchor changed")
source = source.replace(call_old, call_new)

if source.count('"action_scoring_parallelism": 4,') != 1:
    raise RuntimeError("v800 parallelism audit anchor changed")
source = source.replace(
    '"action_scoring_parallelism": 4,',
    '"action_scoring_parallelism": int(fast_core.LAST_PERFORMANCE_AUDIT["worker_count"]),',
)
summary_old = '        "seed_component_counts": seed_summary,\n'
summary_new = (
    '        "seed_component_counts": seed_summary,\n'
    '        "parallel_runtime_audit": fast_core.LAST_PERFORMANCE_AUDIT,\n'
)
if source.count(summary_old) != 1:
    raise RuntimeError("v800 summary audit anchor changed")
source = source.replace(summary_old, summary_new)
source = source.replace('"schema_version": 735,', '"schema_version": 800,')
