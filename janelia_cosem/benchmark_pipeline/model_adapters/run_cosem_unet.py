"""Train/predict the released COSEM-architecture 2D/3D U-Net controls."""
...
def build_cosem_architecture_unet(torch, spatial_dims: int):
    """Build the repository-tracked U-Net used for the reported COSEM controls."""
    ...
    class CosemArchitectureUNet(torch.nn.Module):
        def __init__(self):
            super().__init__()
...
def main() -> None:
    ...
    image = (image - np.percentile(image, 1)) / max(
        1e-6, np.percentile(image, 99.8) - np.percentile(image, 1)
    )
    image = np.clip(image, 0, 1)
    label = sparse > 0
    model = build_cosem_architecture_unet(
        torch, spatial_dims=2 if args.dimension == "2d" else 3
    ).to(device)
    ...
    for _epoch in range(args.epochs):
        ...
        for _ in range(args.steps_per_epoch):
            ...
            xb = torch.from_numpy(
                np.asarray(x_patches, dtype=np.float32)[:, None]
            ).to(device)
            yb = torch.from_numpy(
                np.asarray(y_patches, dtype=np.float32)[:, None]
            ).to(device)
            ...
            weight = torch.tensor(
                min(100.0, neg / max(pos, 1.0)), device=device
            )
            loss = torch.nn.functional.binary_cross_entropy_with_logits(
                logits, yb, pos_weight=weight
            )
            loss.backward()
            optimizer.step()
            ...
            for z0 in starts(image.shape[0], dz, strides[0]):
                for y0 in starts(image.shape[1], xy, strides[1]):
                    for x0 in starts(image.shape[2], xy, strides[2]):
                        ...
                        probabilities[
                            z0:z0 + dz, y0:y0 + xy, x0:x0 + xy
                        ] += prob
                        counts[z0:z0 + dz, y0:y0 + xy, x0:x0 + xy] += 1
    probabilities /= np.maximum(counts, 1)
    tifffile.imwrite(
        args.output,
        (probabilities >= 0.5).astype(np.uint8),
        compression="zlib",
    )
    torch.save(model.state_dict(), args.work_dir / "model.pt")
    write_timing(
        args.output,
        model=f"cosem_{args.dimension}_unet",
        started=started,
        epochs=args.epochs,
        extra={
            "sampler": "direct positive-centered patches",
            "architecture_source": ARCHITECTURE_SOURCE,
            "architecture_implementation": "repository-tracked explicit U-Net",
            "seed": args.seed,
